...
function buildTree(e) {
  let {
      buffer: t,
      nodeSet: w,
      maxBufferLength: S = DefaultBufferLength,
      reused: b = [],
    ...
    A = 0,
    M = 0
  function T(i, n, e, t, r) {
    for (var o, a, { id: s, start: l, end: h, size: c } = x, d = M; c < 0; ) {
      if ((x.next(), -1 == c)) return (o = b[s]), e.push(o), void t.push(l - i)
      if (-3 == c) return void (A = s)
      if (-4 == c) return void (M = s)
  ...
  let i = [],
    r = []
  for (; 0 < x.pos; ) T(e.start || 0, e.bufferStart || 0, i, r, -1)
...
;(EditorState.allowMultipleSelections = allowMultipleSelections),
  (EditorState.tabSize = Facet.define({ combine: e => (e.length ? e[0] : 4) })),
  ...
  scrollIntoView(n) {
    ...
    if (h) {
      ...
      let e = 0,
        ...
        i = 0,
        r = 0
      for (var d of this.view.state.facet(scrollMargins).map(e => e(this.view)))
        d &&
    ...
    if (this.destroyed) this.viewState.state = e
      ...
      try {
        ...
        ;(this.viewState = new ViewState(e)),
          (this.plugins = e.facet(viewPlugin).map(e => new PluginInstance(e))),
  ...
  match(n) {
    ...
    for (let i = 0, e = Math.min(n.length, 200), r = 0; i < e && f < c; ) {
      ...
      let e,
        t =
          S < 255
            ? (48 <= S && S <= 57) || (97 <= S && S <= 122)
              ? 2
              : 65 <= S && S <= 90
              ? 1
              : 0
            : (e = fromCodePoint(S)) != e.toLowerCase()
            ? 1
            : e != e.toUpperCase()
            ? 2
            : 0
...
function fieldSelection(e, t) {
  return EditorSelection.create(
    e.filter(e => e.field == t).map(e => EditorSelection.range(e.from, e.to))
  )
}
function snippet(e) {
  ...
  searchConfigFacet = Facet.define({
    combine(e) {
      return {
        ...
        createPanel:
          (null == (e = e.find(e => e.createPanel)) ? void 0 : e.createPanel) ||
          (e => new SearchPanel(e)),
      }
    },
  })
      ...
      highlight({ query: o, panel: e }) {
        ...
        for (let i = 0, r = s.visibleRanges, n = r.length; i < n; i++) {
          ...
          o.highlight(s.state.doc, e, t, (t, i) => {
            var e = s.state.selection.ranges.some(e => e.from == t && e.to == i)
...
const baseTheme = EditorView.baseTheme({
  ...
  snippets = [
    ...
    snippetCompletion(':= [${log}; LOGGER [0]];', {
      label: 'logger',
      detail: 'log',
      type: 'type',
    }),
    snippetCompletion(
      '-> [matcher; ?? [. [:: [\n\t${"case"}; -> [..[${}]];\n;; add more cases here\n;; ...\n]; matcher];\n;; default case\n-> [${"default case"}]\n][]]\n[${"matcher"}]',
      { label: 'switch', detail: 'block', type: 'keyword' }
    ),
    snippetCompletion('-> [${x}; ${}]', {
      label: 'f(x)',
      detail: 'anonymous',
    ...
    snippetCompletion('-> [${x}; ${i}; ${a}; .. [\n\t${}\n]]', {
      label: 'arrow',
      detail: 'iterator function multiline',
      type: 'type',
    }),
    snippetCompletion('.. [${}]', {
      label: 'block',
    ...
    snippetCompletion(
      ...
      {
        label: 'loop times',
        detail: 'tail recursive loop (backwards)',
        type: 'type',
      }
    ),
    snippetCompletion(
      ...
      {
        label: 'loop range',
        detail: 'tail recursive loop (range)',
        type: 'type',
      }
    ),
    ...
    snippetCompletion('>_ [${array}; ${callback}; ${output}]', {
      label: 'reduce',
      detail: 'iterator',
      type: 'type',
    }),
    snippetCompletion('_< [${array}; ${callback}; ${output}]', {
      label: 'reduce backwards',
      detail: 'iterator',
      type: 'type',
    }),
    ...
    snippetCompletion('>- [${array}; ${predicate}]', {
      label: 'filter',
      detail: 'filter',
      type: 'type',
    }),
    ...
    snippetCompletion('..: [${array}; ${index}; ${value}]', {
      label: 'set',
      detail: 'setter',
      type: 'type',
    }),
    snippetCompletion('@ [${times}; ${callback}]', {
      label: 'routine',
      detail: 'loop',
      type: 'type',
    }),
    snippetCompletion('.-: [${string}; ${separator}]', {
      label: 'split',
      detail: 'transform',
      type: 'type',
    }),
    snippetCompletion('.:* [${array}; ${start}; ${end}]', {
      label: 'slice',
      detail: 'transform',
      type: 'type',
    }),
    snippetCompletion('~:: [${array}; ${dir}]', {
      label: 'quick sort',
      detail: 'transform',
      type: 'type',
    }),
    ...
    snippetCompletion('_. [${array}; ${level}]', {
      label: 'flat',
      detail: 'transform',
      type: 'type',
    }),
